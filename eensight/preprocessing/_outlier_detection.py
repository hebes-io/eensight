...
class SeasonalAD(BaseEstimator):
    """ Outlier detector for time series data using seasonal decomposition.
    Includes adapted code from https://github.com/facebook/prophet 

    Parameters
    ----------
    name: The name of the feature that will be checked for outliers. 
        String.
    ...
    c: float, optional
        ...
        Default: 3.0.
    """
    def __init__(self, name: str, trend: str = 'c',
    ...
    def fit(self, X, y=None):
        ...
        self._set_seasonalities(X.index)
        features = self._gather_all_features(X)
        self.model = sm.RLM(X[self.name], features, missing='drop', M=sm.robust.norms.HuberT()).fit()
        self.is_fitted_ = True
        return self


    def predict(self, X: pd.DataFrame, return_forecast: bool = True):
        check_is_fitted(self, 'is_fitted_')
        X = self._validate_inputs(X)
        features = self._gather_all_features(X)
        pred = self.model.predict(features)

        residuals = X[self.name] - pred 
        ad = InterQuartileRangeAD(c=self.c)
        outliers = ad.fit_detect(residuals)
        # To comply with scikit-learn
        outliers.loc[outliers==1] = -1 # outliers 
        outliers.loc[outliers==0] = 1  # inliners

        if return_forecast:
            return pd.concat([pred.to_frame('yhat'), outliers.to_frame('outliers')], axis=1)
        else:
             return outliers.to_frame('outliers')


    def fit_predict(self, X, y=0, return_forecast=True):
        return self.fit(X, y).predict(X, return_forecast=return_forecast)
