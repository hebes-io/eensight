...
class ClusterFeatures(TransformerMixin, BaseHeterogeneousEnsemble):
    ...
    def transform(self, X: pd.DataFrame):
        ...
        if self.remainder == "passthrough":
            if (self.output_name in X.columns) and not self.replace:
                raise ValueError(f"Found common column name `{self.output_name}`")
            elif self.output_name in X.columns:
                X = X.drop(self.output_name, axis=1)
            pred = pd.concat((X, pred), axis=1)
